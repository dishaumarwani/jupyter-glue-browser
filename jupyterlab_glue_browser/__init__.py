...
class GlueResource:
    ...
    def __init__(self, config):
        ...
        self.glue_resource = boto3.client(
            'glue'
        )
...
class GlueHandler(APIHandler):
    """
    Handles requests for getting Glue objects
    """
    ...
    def get(self, path=''):
        ...
        try:
            if not self.glue_resource:
                self.glue_resource = GlueResource(self.config).glue_resource

            if path == "/":
                # requesting the root path, just return all buckets
                all_databases = self.glue_resource.get_databases()

                x = all_databases["DatabaseList"]

                ## Extract all database names
                databases = lambda x: x["Name"]
                all_databases = list(map(databases, all_databases["DatabaseList"]))
                result = [{
                    'name': database,
                    'path': database,
                    'type': 'directory'
...
def _jupyter_server_extension_paths():
    return [{
        'module': 'jupyterlab_glue_browser'
    }]
...
def load_jupyter_server_extension(nb_server_app):
    ...
    endpoint = url_path_join(base_url, 'glue')
    handlers = [
        (url_path_join(endpoint, 'auth') + "(.*)", AuthHandler),
        (url_path_join(endpoint, 'files') + "(.*)", GlueHandler)
    ]
